...
class Network:
    ...
    def _get_edges(
        ...
        include_workflow_inputs: bool = False,
        ...
        for node_key, node in nodes.items():
            ...
            for other_key, input_node in node._input_nodes.items():
                if not include_workflow_inputs and isinstance(
                    input_node, WorkflowInput
                ):
                    continue
    ...
    def to_nx(
        ...
        edges = self._get_edges(
            workflow,
            include_workflow_inputs=include_workflow_inputs,
    ...
    def to_pyvis(
        ...
        show_workflow_inputs: bool = False,
        edge_labels: bool = True,
        node_help: bool = True,
        notebook: bool = False,
        hierarchial: bool = True,
        inputs_props: Dict[str, Any] = {},
        node_props: Dict[str, Any] = {},
        leafs_props: Dict[str, Any] = {},
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "You need to install the 'networkx', 'pyvis' and 'matplotlib' packages to visualize workflows."
            )
        ...
        graph = self.to_nx(
            self._workflow,
            include_workflow_inputs=show_workflow_inputs,
            edge_labels=edge_labels,
        )
        ...
        inputs_are_first_gen = all(
            node in self._workflow.dryrun_nodes.inputs for node in topo_gens[0]
        )

        output_node_key = "output"
        output_is_last_gen = (
            len(topo_gens[-1]) == 1 and output_node_key == topo_gens[-1][0]
        )
        ...
        def _get_node_help(node: Union[Node, Type[Node]]):
            if isinstance(node, Node):
                ...
                sig = "\n".join(
                    str(param) for param in node_cls.__signature__.parameters.values()
                )
        ...
        for i, nodes in enumerate(topo_gens):
            ...
            font = {}
            if auto_text_color and shape in [
                "ellipse",
                "circle",
                "database",
                "box",
        ...
        net = visNetwork(
            notebook=notebook, directed=True, layout=layout, cdn_resources="remote"
        )
        net.from_nx(graph)
    ...
    def _show_pyvis(net: "pyvis.Network", notebook: bool, to_export: Union[bool, None]):
        ...
        if notebook:
            ...
            if to_export:
                ...
                obj = HTML(
                    f"""<iframe height='{net.height}' srcdoc="{escaped_html}" style="width:100%"></iframe>"""
                )
            else:
                obj = HTML(
                    f'<div style="height:{net.height}">{html_text}</div>',
    ...
    def visualize(
        self,
        colorscale: str = "viridis",
        show_workflow_inputs: bool = True,
        edge_labels: bool = True,
        node_help: bool = True,
        notebook: bool = False,
        hierarchial: bool = True,
        ...
        net = self.to_pyvis(
            ...
            show_workflow_inputs=show_workflow_inputs,
            edge_labels=edge_labels,
            node_help=node_help,
            notebook=notebook,
...
class WorkflowNodes:
    ...
    def from_workflow_run(
        ...
        return cls(
            inputs=inputs, workers=workers, output=output, named_vars=_named_vars
        )
    ...
    def from_node_tree(cls, output_node):
        ...
        for k, node in workers.items():
            ...
            node_inputs = {
                param_k: WorkflowInput(
                    input_key=f"{node.__class__.__name__}_{param_k}",
                    value=node.inputs[param_k],
                )
                for param_k, v in node.inputs.items()
                if not (
                    isinstance(v, Node)
                    or param_k == node._args_inputs_key
                    or param_k == node._kwargs_inputs_key
                )
            }
            ...
            with temporal_context(lazy=True):
                new_workers[k] = node.__class__().update_inputs(
            ...
            wf_inputs.update(
                {
                    f"{node.__class__.__name__}_{param_k}": v
                    for param_k, v in node_inputs.items()
                }
            )
        ...
        for k, node in new_workers.items():
            ...
            for param_k, v in node.inputs.items():
                if param_k == node._args_inputs_key:
                    new_node_inputs[param_k] = [
                        old_to_new[id(n)] if isinstance(n, Node) else n for n in v
                    ]
                elif param_k == node._args_inputs_key:
                    new_node_inputs[param_k] = {
                        k: old_to_new[id(n)] if isinstance(n, Node) else n
                        for k, n in v.items()
                    }
        ...
        return cls(
            inputs=wf_inputs, workers=new_workers, output=new_output, named_vars={}
        )
    ...
    @staticmethod
    def gather_from_inputs_and_output(
        inputs: Sequence[WorkflowInput], output: WorkflowOutput
    ) -> Dict[str, Node]:
    ...
    def copy(self, inputs: Dict[str, Any] = {}) -> "WorkflowNodes":
        ...
        for input_k, input_node in self.inputs.items():
            new_inputs[input_k] = input_node.__class__(
                input_key=input_node.input_key,
                value=inputs.get(input_k, input_node.value),
            )

        # Now create a copy of the worker nodes
        old_ids_to_key = {id(node): key for key, node in self.workers.items()}
        new_output = []
        new_workers = {}
        ...
        def copy_node(node):
            ...
            new_node_inputs = node.map_inputs(
                inputs=node.inputs, only_nodes=True, func=copy_node
            )
...
class Workflow(Node):
    ...
    def from_node_tree(cls, output_node: Node, workflow_name: Union[str, None] = None):
        ...
        signature = inspect.Signature(
            parameters=[
                inspect.Parameter(
                    inp.input_key, inspect.Parameter.KEYWORD_ONLY, default=inp.value
                )
                for inp in dryrun_nodes.inputs.values()
            ]
        )

        def function(*args, **kwargs):
            raise NotImplementedError(
                "Workflow class created from node tree. Calling it as a function is not supported."
            )
    ...
    def __init_subclass__(cls):
        ...
        def assign_workflow_var(value: Any, var_name: str):
            ...
            if var_name in named_vars:
                raise ValueError(
                    f"Variable {var_name} has already been assigned a value, in workflows you can't overwrite variables."
                )
        ...
        cls.dryrun_nodes = WorkflowNodes.from_workflow_run(
            inputs=inps, output=out, named_vars=named_vars
        )
    ...
    def find_node_key(cls, node, *args) -> str:
        ...
        raise ValueError(
            f"Could not find node {node} in the workflow. Workflow nodes {cls.dryrun_nodes.items()}"
        )
...
def nodify_func(
    ...
    if len(decorators) > 0:
        warn(
            f"Decorators are ignored for now on workflow creation. Ignoring {len(decorators)} decorators on {func.__name__}"
        )
